...
\noindent
\textbf{How to submit:} 
Gradescope (accessible by logging into \url{https://www.gradescope.com/} with your BU credentials) should already have our class list; however, if you are not in the gradescope class list, use entry code G675B7 to add yourself to the class. 

We expect you to submit a solutions pdf. Don't forget to select the pages for each problem in Gradescope.

\medskip \noindent
\textbf{Why type up your solutions:} The quality of typed up solutions is generally higher than the quality of handwritten ones, because you are able to edit your solutions and improve them as you write. It is okay to use any document creation system you want, as many of them now support inserting math. 

\medskip \noindent
\textbf{Using \LaTeX}. We highly recommend learning \LaTeX\ to make it easier to write up your solutions.  We will provide source files for each problem set. \LaTeX\ is like a programming language, but for producing documents. Just like there are many environments for producing Java or Python or C programs, there are many environments you can use to produce \LaTeX. You should choose one.  If you want to work in the cloud through your browser, we recommend \href{https://crixet.com/}{Crixet} or \href{https://overleaf.com}{Overleaf}. If you want to have a local installation, this page \url{https://www.latex-project.org/get/} tells you how to install \LaTeX.  For local installations, consider recommend MacTeX for a Mac and MiKTeX for Windows. There are many good editors; I like TeXShop for Mac. TeXstudio and TeXworks are also good options, available for different operating systems. VS Code also supports \LaTeX. Search the web for ``getting started with Latex'' if these pointers are not enough. It is okay to hand-draw diagrams to insert into \LaTeX. 

\medskip\noindent
\textbf{If you prefer to handwrite:} 
If you handwrite your solutions, please use the Gradescope app to scan and submit, per instructions at {\tiny \url{https://guides.gradescope.com/hc/en-us/articles/22016028459789-Using-the-Gradescope-Mobile-App-for-Students}}.
 \medskip
 
 
 
\medskip\noindent\textbf{How to Approach the Problem Sets}
The best way to tackle the problem sets is to do them over time rather than once a week. You wouldn't starve for six days and then attempt to consume your weekly calorie needs on the seventh. Similarly, don't deprive your brain of thinking about CS 538 for six days and then attempt to solve the problem set on the seventh.

Discussions help you start the problem set. Every homework problem relates to something we covered; if unsure how to start, look through what we covered in the last few lectures.

We encourage you to work with others, subject to limitations of the collaboration policy below. We also encourage you to come to office hours (see the schedule in a pinned Piazza post), where you can work with others with the help of the course staff.
\newpage
...
\begin{mybox}
  \begin{remark}
    \label{remark:rem1}
    There are multiple ways to rewrite Definition~\ref{def:ots}, because there are many ways to say ``the output of $\attack$ is uniformly distributed.''  We do so now.
    
     Then Definition~\ref{def:ots} is equivalent to saying that for any message $\msg$,
    {\small 
    \begin{align*}
...
\begin{ppart}
    Prove that this scheme does {\it not} have one-time secrecy as defined in Definition~\ref{def:ots}.
...
\begin{solution}
  ...
  Looking back at the Definition~\ref{def:ots}, $\Sigma$ has OTS IFF for every message $\msg \in \M$ and every ciphertext $\ctext \in \C$,
  \begin{equation*}
...
\begin{ppart}
Prove that this scheme does \emph{not} have Shannon secrecy as defined in Definition~\ref{def:shannon}.
\end{ppart}
...
Recall the scheme $\Sigma$, and the definitions of each of its parameters. Looking at Definition~\ref{def:shannon}, $\Sigma$ has Shannon secrecy IFF for every pair of messages $\msg_0, \msg_1 \in \M$ and every ciphertext $\ctext \in \C$,
\begin{equation*}
