...
\begin{solution}
Like again, these are one of those convoluted math-substitution problems, where I just gotta go back to the rules to solve it. But I think this particular question does rely on Fermat's little theorem.\\
...
\begin{solution}
The solution to this problem will be, like the hint says, doing via a contradiction. Basically, assume that $a^{(p-1)/2} \pmod 1$, and eventually it'll become clear that this can't be.\\

\noindent Assume that $a^{(p-1)/2} \equiv_p 1$ but that$a$ is \textbf{not} a square modulo $p$.\\

\noindent Because of the property of $g$, we can write $a = g^x$ for some $x \in \Z_{p-1}$.\\

\noindent By (a), we alreay know that that $a$ is a square IFF $x$ is even. Since I assumed $a$ is not a square, then $x$ must be odd. Therefore, I can write $x = 2k + 1$ for some int $k$.\\

\noindent Now calculate $a^{(p-1)/2}$:
\begin{equation}
a^{(p-1)/2} \equiv_p (g^x)^{(p-1)/2} \equiv_p g^{x \cdot \frac{p-1}{2}} \equiv_p g^{(2k+1) \cdot \frac{p-1}{2}}.
\end{equation}

\noindent Expanding the exponent:
\begin{equation}
g^{(2k+1) \cdot \frac{p-1}{2}} \equiv_p g^{k(p-1) + \frac{p-1}{2}} \equiv_p g^{k(p-1)} \cdot g^{(p-1)/2}.
\end{equation}

\noindent And now, by Fermat's Little Theorem:
\begin{equation}
a^{(p-1)/2} \equiv_p 1 \cdot g^{(p-1)/2} \equiv_p g^{(p-1)/2}.
\end{equation}

\noindent However, since $g$ has multiplicative order $p-1$ in $\Z_p^*$, then it is also the case that $g^m \equiv_p 1$ IFF $(p-1) \mid m$. And since $0 < (p-1)/2 < p-1$, then we get:
\begin{equation}
g^{(p-1)/2} \not\equiv_p 1.
\end{equation}

\noindent This contradicts the initial assumption that $a^{(p-1)/2} \equiv_p 1$. Therefore, it must be false.\\

\noindent Thus, if $a^{(p-1)/2} \equiv_p 1$, then $a$ is a square modulo $p$.
\end{solution}
...
\end{ppart}
\begin{solution}
In order to prove this, I will essentially have to do 2 things. Firstly, I'll nee to verify the 2nd root. And then, I'll need to prove its distinctness.\\

\noindent I'll firstly prove that $g^{x+(p-1)/2}$ is also a square root of $a$. By computing the square of this value:
\begin{equation}
\left(g^{x+(p-1)/2}\right)^2 \equiv_p g^{2\left(x+(p-1)/2\right)} \equiv_p g^{2x + (p-1)}.
\end{equation}

\noindent By the laws of exponents:
\begin{equation}
g^{2x + (p-1)} \equiv_p g^{2x} \cdot g^{p-1} \equiv_p \left(g^x\right)^2 \cdot g^{p-1}.
\end{equation}

\noindent And by Fermat's Little Theorem, since $g \in \Z_p^*$, then $g^{p-1} \equiv_p 1$. Therefore:
\begin{equation}
\left(g^x\right)^2 \cdot g^{p-1} \equiv_p \left(g^x\right)^2 \cdot 1 \equiv_p a.
\end{equation}

\noindent Thus, $\left(g^{x+(p-1)/2}\right)^2 \equiv_p a$. \\

\noindent Now, I'll show how these two square roots are distinct. Assume, for the sake of contradiction, that they are in fact the same:
\begin{equation}
g^x \equiv_p g^{x+(p-1)/2}.
\end{equation}

\noindent Now, since $g^x \in \Z_p^*$, it has a multiplicative inverse modulo $p$. By multiplying both sides by $(g^x)^{-1}$, we get:
\begin{equation}
1 \equiv_p g^{(p-1)/2}.
\end{equation}

\noindent But remember how $g$ is a generator of $\Z_p^*$ with multiplicative order $p-1$. THen by the defintion of its order, $g^k \equiv_p 1$ IFF $(p-1) \mid k$. And since $p > 2$, we know that $0 < (p-1)/2 < p-1$, meaning that $(p-1)$ does not divide $(p-1)/2$. Thusly:
\begin{equation}
g^{(p-1)/2} \not\equiv_p 1.
\end{equation}

\noindent And this is where the contradiction occurs. That then means, ultimately, that the two square roots must be distinct:
\begin{equation}
g^x \not\equiv_p g^{x+(p-1)/2}.
\end{equation}
...
\begin{solution}
Now in order to prove this, this also involves 2 parts. The first is showing how $g^{(p-1)/2} \equiv_p -1$. Then, I'll have to show how $b^{(p-1)/2} \equiv_p -1$ for non-squares specifically.\\

\textbf{Getting $g^{(p-1)/2}\equiv_p -1$}

\noindent Ok so we start off knowing that $g^{x+(p-1)/2}\equiv_p -g^x$. Then, by the laws of exponents, I can rewrite the left-hand side to be the following:
\begin{equation}
g^{x+(p-1)/2} \equiv_p g^x \cdot g^{(p-1)/2}.
\end{equation}

\noindent Now subbing this into the given relation:
\begin{equation}
g^x \cdot g^{(p-1)/2} \equiv_p -g^x.
\end{equation}

\noindent And by the properties of $g$, it is a fact that $g^x \in \Z_p^*$, which means $g^x$ has a multiplicative inverse modulo $p$. So then, by multiplying both sides by $(g^x)^{-1}$:
\begin{equation}
g^{(p-1)/2} \equiv_p -1.
\end{equation}
...
\textbf{$b^{(p-1)/2} \equiv_p -1$ for any non-square $b$.}

\noindent Let $b \in \Z_p^*$ be a non-square. Now by the properties of $g$, we can write $b = g^y$ for some $y \in \Z_{p-1}$.\\

\noindent By (a), I alr know that $b$ is a square IFF $y$ is even. And since $b$ is a non-square, $y$ must be odd. So I'll just write $y = 2k + 1$ for some int $k$.\\

\noindent Now determine $b^{(p-1)/2}$:
\begin{equation}
b^{(p-1)/2} \equiv_p (g^y)^{(p-1)/2} \equiv_p g^{y \cdot \frac{p-1}{2}} \equiv_p g^{(2k+1) \cdot \frac{p-1}{2}}.
\end{equation}

\noindent Expand the exponent:
\begin{equation}
g^{(2k+1) \cdot \frac{p-1}{2}} \equiv_p g^{k(p-1) + \frac{p-1}{2}} \equiv_p g^{k(p-1)} \cdot g^{(p-1)/2}.
\end{equation}

\noindent And by Fermat's Little Theorem, we get $g^{p-1} \equiv_p 1$:
\begin{equation}
g^{k(p-1)} \equiv_p (g^{p-1})^k \equiv_p 1^k \equiv_p 1.
\end{equation}

\noindent Subbing this back:
\begin{equation}
b^{(p-1)/2} \equiv_p 1 \cdot g^{(p-1)/2} \equiv_p g^{(p-1)/2}.
\end{equation}

\noindent And from the first part, we showed that $g^{(p-1)/2} \equiv_p -1$. So then therefore:
\begin{equation}
b^{(p-1)/2} \equiv_p -1.
\end{equation}

\noindent This proves how, for any $a \in \Z_p^*$,
\begin{itemize}
\item If $a$ is a square, then $a^{(p-1)/2} \equiv_p 1$ (from (b)).
\item And how, if $a$ is a non-square, then $a^{(p-1)/2} \equiv_p -1$.
\end{itemize}
